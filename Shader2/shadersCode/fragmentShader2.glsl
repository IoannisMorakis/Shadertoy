...
// --- Original Shadertoy Code (Neonwave sunrise - 7dyyRy) ---
...
// License: MIT, author: Inigo Quilez, found: https://iquilezles.org/articles/spherefunctions/spherefunctions.htm
...
vec4 moon(vec3 ro, vec3 rd) {
    ...
    for (int i = -limit; i <= limit; ++i) {
        ...
        // float fft   = texture(iChannel0, cp).x; // REMOVED
        float fft   = 0.5; // Fixed value (iChannel0 removed)
...
vec3 skyColor(vec3 ro, vec3 rd) {
    ...
    // FIX: Apply the approximation function component-wise
    col += vec3(tanh_approx(lcol.r*li), tanh_approx(lcol.g*li), tanh_approx(lcol.b*li));
...
vec3 color(vec3 ww, vec3 uu, vec3 vv, vec3 ro, vec2 p) {
    ...
    // Playing around with rdd can give interesting distortions
    float rdd = 2.0;
    vec3 rd = normalize(p.x*uu + p.y*vv + rdd*ww);
    vec3 nrd = normalize(np.x*uu + np.y*vv + rdd*ww);
    const float planeDist = 1.0;
    const int furthest = 12;
    // FIX: Replaced max(int, int) with a ternary operator to comply with strict GLSL ES 1.0
    const int fadeFrom = (furthest-2) > 0 ? (furthest-2) : 0;
...
v = max(v, 0.0);
// v *= 0.6f; // 'f' suffix removed
v *= 0.6;
// float a = 2.51f; // 'f' suffix removed
float a = 2.51;
// float b = 0.03f; // 'f' suffix removed
float b = 0.03;
// float c = 2.43f; // 'f' suffix removed
float c = 2.43;
// float d = 0.59f; // 'f' suffix removed
float d = 0.59;
// float e = 0.14f; // 'f' suffix removed
float e = 0.14;

...
uniform vec4 iMouse; // Using iMouse uniform for mouse input
varying vec2 vUv;


// --- Original Shadertoy Code (Adapted) ---
...
void initIcosahedron() {//setup folding planes and vertex
	...
	pbc=normalize(pbc);	pca=normalize(pca);//for slightly better DE.
    // In reality it's not necesary to apply normalization :) 
...
void main()
{
    // 1. Shadertoy uniform initializations
    t = iTime;
    t = mod(t/4., 1.);    

    initIcosahedron();
    
    // 2. Fragment Coordinate and Mouse Conversion
    // Shadertoy: p = (-iResolution.xy + 2.0*fragCoord.xy)/iResolution.y;
    // three.js vUv is (0,0) to (1,1). We convert it to Normalized Device Coordinates (NDC) 
    // where the center is (0, 0) and edges are +/- aspectRatio on X and +/- 1 on Y.
    vec2 fragCoord = vUv * iResolution.xy;
    vec2 p = (2.0 * fragCoord - iResolution.xy) / iResolution.y; 
    
    // Convert iMouse.xy to normalized m (0 to 1)
    ...
    // 3. Final Output (Replaces fragColor)
